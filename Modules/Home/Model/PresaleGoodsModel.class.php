...
class PresaleGoodsModel {
	...
	public function getIndexPresaleGoods($is_index_show_pa =1)
    {
        ...
        if( !empty($community_goods) )
        {
            ...
            foreach($community_goods as $val)
            {
                ...
                $goods_presale = M('lionfish_comshop_goods_presale')->where(['goods_id' => $val['id'] ])->find();
    ...
    public function getOrderPresaleInfo( $order_id )
    {
        ...
        {
            if(  ($order_presale['presale_limit_balancepaytime'] == 1 && ( $order_presale['presale_balance_beginpaytime'] < $now_time && $order_presale['presale_balance_paytime'] > $now_time ) ) || $order_presale['presale_limit_balancepaytime'] == 1 )
            {
    ...
    public function cronPresaleMsg()
    {
        ...
        $sql = "SELECT * FROM ".C('DB_PREFIX')."lionfish_comshop_order_presale where state = 1 and is_notify = 0 and presale_balance_beginpaytime <= {$notify_time} order by id asc limit 1000 ";
