...
<form action="<?php echo U('order/kdniao_order');?>" method="post" id="export_form" class="form-horizontal form-search " role="form">
	...
	<input type="hidden" name="print_type" value="<?php echo ($type); ?>" id="print_type"/>
...
<script>
	$(function(){
		$('#sub_orderexport').click(function(){
			...
			if(print_type != 'one'){
				...
				$.ajax({
					url: "<?php echo U('order/kdniao_print_orders');?>",
					type:'post',
					dataType:'json',
					data:{print_type:print_type,order_id:order_id,express_code:express_code},
					success:function(info){
						layer.closeAll();
						$.post("<?php echo U('order/kdn_orders_info');?>", {order_ids:info.order_ids}, function(shtml){
							layer.open({
								title: "快递面单",
								type: 1,
								area: ['450px', '700px'],
				...
				$.ajax({
					url: "<?php echo U('order/kdniao_order');?>",
					type:'post',
					dataType:'json',
	...
	function kdniao_set(){
		...
		layer.confirm(html, {
			...
			window.location.href = "<?php echo U('express/config');?>";
		}, function(){
			layer.closeAll();
		});
	}
