			...
			<form action="" method="get" class="form-horizontal form-search layui-form" role="form">
				...
				<div class="layui-form-item">
				  <div class="layui-inline">
					...
					<div class="layui-input-inline" >
						<input type="text" class="layui-input"  name="keyword" value="<?php echo ($keyword); ?>" placeholder="请输入会员ID,昵称或手机号"/>
					...
					<div class="layui-input-inline" >
						<select name="level_id" class="layui-input layui-unselect">
							...
							<option value="<?php echo ($level['id']); ?>" <?php if( $gpc['level_id'] == $level['id']){ ?>selected<?php } ?>><?php echo ($level['levelname']); ?></option>
							<?php } ?>
						</select>
					</div>
					<div class="layui-input-inline">
						<select name="groupid" class="layui-input layui-unselect">
							...
							<option value="<?php echo ($group['id']); ?>" <?php if( $gpc['groupid'] == $group['id']){ ?>selected<?php } ?>><?php echo ($group['groupname']); ?></option> 
							<?php } ?>
						</select>
					</div>
					
					<div class="layui-input-inline">
						<select name="card_id" class="layui-input layui-unselect">
					...
					<div class="layui-input-inline">
						...
						<button type="submit" name="export" value="1" class="layui-btn layui-btn-sm btn_export">导出</button>
					</div>
				  </div>
				</div>
			</form>
				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="page-table-header">
							...
							<div class="btn-group">
								<button class="btn btn-default btn-sm btn-operation" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php echo U('user/deleteuser');?>">
									删除
								</button>
								<button class="btn btn-default btn-sm " type="button" data-toggle="batch-group" data-href="<?php echo U('user/changelevel', array('toggle' => 'group'));?>" > <i class="icow icow-fenzuqunfa"></i>修改分组</button>
								<button class="btn btn-default btn-sm " type="button" data-toggle="batch-level" data-href="<?php echo U('user/changelevel', array('toggle' => 'level'));?>" ><i class="icow icow-cengjiguanli"></i> 修改等级</button>
						...
						<table class="table table-responsive" lay-even lay-skin="line" lay-size="lg">
							<thead>
							 <tr>
								...
								<th style="">等级/分组/付费会员卡</th>
								<th>
									<!-- https://sqtg.shiziyu888.com/seller.php?c=user&a=index&keyword=1&level_id=15&groupid=16&card_id=16&searchtime=create&time%5Bstart%5D=2020-11-10+00%3A00&time%5Bend%5D=2020-11-26+23%3A59 -->	
									<!--https://sqtg.shiziyu888.com/seller.php?s=/user/index/sortfield/seller_count/keyword/1/searchtime/create/sort_starttime/1606320000/sort_endtime/1606406340	-->								
							...
							<?php foreach( $list as $row ){ ?>
								<tr>
									<td style="position: relative; ">
										<input type='checkbox' lay-skin="primary" value="<?php echo ($row['member_id']); ?>" class="checkone" name="item_checkbox"/>
									...
									<td style="overflow: visible">
										<div rel="pop" style="display: flex" >
										   ...
										   <span style="display: flex;flex-direction: column;justify-content: center;align-items: flex-start;padding-left: 5px">
											   <span class="nickname">
												   <?php if( empty($row['username'])){ ?>未更新<?php  }else{ echo ($row['username']); ?><br/>
								   ...
								   <?php if($commiss_level > 0){ ?>
									<td> 
										<input type="checkbox" name="" lay-filter="statewsitch" data-href="<?php echo U('distribution/become_agent_check',array('id'=>$row['member_id']));?>" <?php if($row['comsiss_flag']==1 && $row['comsiss_state']==1){ ?>checked<?php } ?> lay-skin="switch" lay-text="是|否">
									...
									<td>
										...
										<input type="checkbox" name="" lay-filter="statewsitch" data-href="<?php echo U('User/agent_shenhe',array('id'=>$row['member_id']));?>" <?php if($row['is_apply_state']==1 ){ ?>checked<?php } ?> lay-skin="switch" lay-text="是|审核通过">
										<input type="checkbox" name="" lay-filter="statewsitch" data-href="<?php echo U('User/agent_unshenhe',array('id'=>$row['member_id']));?>" <?php if($row['is_apply_state']==1 ){ ?>checked<?php } ?> lay-skin="switch" lay-text="拒绝审核|拒绝审核">
									...
									<td style="overflow:visible;text-align: center;">
										...
										<div class="btn-group">
											<a class="layui-btn layui-btn-xs" href="<?php echo U('user/detail',array('id' => $row['member_id'], 'ok' => 1));;?>" title="">
												<span data-toggle="tooltip" data-placement="top" title="" data-original-title="会员详情">
													会员详情
												</span>
											</a>
											<a class="layui-btn layui-btn-xs" href="<?php echo U('order/index', array('type' => 'all','searchfield'=>'member_id' ,'keyword' => $row['member_id'] ));?>"
											   title=''>
												  会员订单
											</a>
											...
											<button class="layui-btn layui-btn-xs deldom" type="button" data-toggle='ajaxRemove' data-href="<?php echo U('user/deleteuser',array('id' => $row['member_id']));;?>" data-confirm="确定要删除该会员吗？">
