	...
	<div class="layui-card">
		...
		<div class="layui-tab layui-tab-brief" >
		  <ul class="layui-tab-title">
				<li  <?php if(empty($type) || $type=='all'){ ?>class="layui-this"<?php } ?>><a href="<?php echo U('group/goods');?>">全部商品（<?php echo ($all_count); ?>）</a></li>
                <li  <?php if($type=='saleon'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('group/goods',array('type'=>'saleon'));?>">出售中（<?php echo ($onsale_count); ?>）</a></li>
                <li <?php if($type=='stock_notice'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('group/goods',array('type'=>'stock_notice'));?>">库存预警（<?php echo ($stock_notice_count); ?>）</a></li>
                <li <?php if($type=='getdown'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('group/goods',array('type'=>'getdown'));?>">已下架（<?php echo ($getdown_count); ?>）</a></li>
              
				<?php if($is_open_shenhe == 1){ ?>
				<li <?php if($type=='warehouse'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('group/goods',array('type'=>'warehouse'));?>">仓库（<?php echo ($warehouse_count); ?>）</a></li>
               <?php } ?>
			    <li <?php if($type=='recycle'){ ?>class="layui-this"<?php } ?> ><a href="<?php echo U('group/goods',array('type'=>'recycle'));?>">回收站（<?php echo ($recycle_count); ?>）</a></li>
					...
					<div class="col-md-12">
						...
						<div class="page-table-header">
							...
							<span class="pull-right"> 
								<a href="<?php echo U('group/addgoods', array('ok' =>1));?>" class="layui-btn layui-btn-sm"><i class="fa fa-plus"></i> 添加商品</a>
							</span>
							<div class="btn-group">
								<?php if($is_index){ ?>
						...
						<table class="table table-responsive" lay-even lay-skin="line" lay-size="lg">
							...
							<tr>
							<td>
								<input type='checkbox' name="item_checkbox" data_is_all_sale="<?php echo ($item['is_all_sale']); ?>"  class="checkone" lay-skin="primary" value="<?php echo ($item['id']); ?>"/>
							</td>
							<td style="text-align:center;">
								<?php echo ($item['id']); ?>
							</td>
							<td>
								<a href="<?php echo U('group/editgoods', array('id' => $item['id'],'goodsfrom'=>$goodsfrom,'page'=>$page));?>">
							...
							<td >&yen;
								<?php if( $item['hasoption']==1 ){ ?>
									<?php echo ($item['price_arr']['pin_price']); ?> <?php if( isset($item['price_arr']['max_pinprice']) ){ ?>~&yen;<?php echo ($item['price_arr']['max_pinprice']); } ?>
								<?php  }else{ ?>
									<a href='javascript:;' data-toggle='ajaxEdit' data-href="<?php echo U('goods/change',array('type'=>'price','id'=>$item['id']));?>" >
									<?php echo ($item['price_arr']['pin_price']); ?>
									</a>
							   <?php } ?>
							</td>
						...
						<?php } ?>
							...
							<?php if($is_index){ ?>
							<td >
								<input type="checkbox" name="" lay-filter="is_index_showsitch" data-href="<?php echo U('goods/change',array('type'=>'is_index_show','id'=>$item['id']));?>" <?php if( $item['is_index_show']==1 ){ ?>checked<?php  }else{ } ?> lay-skin="switch" lay-text="主页推荐|未推荐">
							...
							<?php } ?>
								...
								<td  style="overflow:visible;position:relative">
									...
									<a  class='layui-btn layui-btn-xs' href="<?php echo U('group/editgoods', array('id' => $item['id'],'ok'=>1,'page'=>$page));?>"  >
									...
									<a href="javascript:;" class="js-clip layui-btn layui-btn-xs" data-url="/lionfish_comshop/moduleA/pin/goodsDetail?id=<?php echo ($item['id']); ?>"><i class="layui-icon">&#xe64c;</i>小程序链接</a>
								</td>
							</tr>
							<?php } ?>
							</tbody>
...
var heads_page = 1;
		...
		if(head_id_arr.length > 0)
		{
			...
			if(is_tip_notice  == 1) {
				layer.confirm('分配售卖团中的商品中有部分商品开启了“<span style="color:red;">所有团长可售</span>”，请确认是否已经取消了“所有团长可售”，未取消的情况下部分商品分配设置可能无效！', function(index){
					$.post("<?php echo U('goods/ajax_batchheads');?>", {
						'goodsids': goodsids,
   ...
   $("body").delegate(".cancle","click",function(){
	...
	$("body").delegate(".modal-group-head","click",function(){
		...
		if(is_tip_notice == 1){
			layer.confirm('分配售卖团长分组中的商品中有部分商品开启了“<span style="color:red;">所有团长可售</span>”，请确认是否已经取消了“所有团长可售”，未取消的情况下部分商品分配设置可能无效！', function(index) {
				$.post("<?php echo U('goods/ajax_batchcates_headgroup');?>", {
					'goodsids': goodsids,
					'groupid': group_heads,
					'is_clear_old': is_clear_old,
    ...
    $('.modal-time').click(function () {
        ...
        $.post("<?php echo U('goods/ajax_batchtime_pintuan');?>",{'goodsids':goodsids,'begin_time': begin_time,'end_time':end_time}, function (ret) {
            if (ret.status == 1) {
                $('#batch_time').hide();
                layer.msg('设置成功');
                window.location.reload();
