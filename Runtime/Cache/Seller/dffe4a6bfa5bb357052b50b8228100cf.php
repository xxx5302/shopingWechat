            ...
            <form action="" method="get" class="form-horizontal form-search layui-form" role="form">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-inline" style="width:280px;">
    				    <input type="text" class="layui-input" name="keyword" value="<?php echo ($keyword); ?>" placeholder="店铺姓名/手机号/供应商名称"/>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="page-table-header">
        					...
        					<span class="pull-right">
        						<a href="<?php echo U('supply/addsupply', array('ok' => 1));?>" class="layui-btn layui-btn-sm"><i class="fa fa-plus"></i> 添加供应商</a>
        					</span>
                            <div class="btn-group">	
        						<a class='btn btn-default btn-sm btn-op btn-operation'  data-toggle='batch' data-href="<?php echo U('supply/agent_check',array('state'=>1,'ok' => 1));?>"  data-confirm='确认要审核通过?'>
        							<i class="icow icow-shenhetongguo"></i>审核通过
        						</a>
        						<a class='btn btn-default btn-sm btn-op btn-operation'  data-toggle='batch' data-href="<?php echo U('supply/agent_check',array('state'=>0,'ok' => 1));?>" data-confirm='确认要取消审核?'>
        							<i class="icow icow-yiquxiao"></i>取消审核</a>
        						<a style="display:none;" class="btn btn-default btn-sm btn-op btn-operation" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php echo U('supply/deletehead');?>">
        					...
        					<?php foreach( $list as $row ){ ?>
        						<tr>
                                    <td style="position: relative; ">
                                        <input type='checkbox' value="<?php echo ($row['id']); ?>" name="item_checkbox" lay-skin="primary"/>
        							</td>
        							<td>
        								<?php echo ($row['id']); ?>
        							</td>
									...
									<td>
										...
										<?php if( $row['state'] == 0 ){ ?>
												...
												<a class='op layui-btn layui-btn-xs'  data-toggle="ajaxModal" href="javascript:;" data-href="<?php echo U('supply/agent_check_first', array('id' => $row['id'],'type'=>$row['type'] ));?>"
												   data-href2="<?php echo U('supply/agent_check_first', array('id' => $row['id'] ));?>">
											...
											<input type="checkbox" name="" lay-filter="statewsitchunable" data-href="<?php echo U('supply/agent_check', array('id'=>$row['id']));?>" <?php if($row['state']==1){ ?>checked<?php } ?> lay-skin="switch" lay-text="已拒绝|拒绝">
											
										<?php }else if( $row['state'] == 1 ){ ?>
											<input type="checkbox" name="" lay-filter="statewsitch" data-href="<?php echo U('supply/agent_check', array('id'=>$row['id']));?>" <?php if($row['state']==1){ ?>checked<?php } ?> lay-skin="switch" lay-text="启用|禁用">
										
										<?php }else if( $row['state'] == 2 ){ ?>
												<p style="color: red;">已拒绝</p>
											<a  data-toggle="ajaxModal" href="javascript:;" data-href="<?php echo U('supply/agent_check_first', array('id' => $row['id']));?>" >
									...
									<td>
										<?php if( $row['type'] == 1 ){ ?>
											...
											<?php if( $row['state'] == 1 ){ ?>
												<input type="checkbox" name="" lay-filter="wsitchmobilemanage" data-href="<?php echo U('supply/agent_mobilemanage', array('id'=>$row['id']));?>" <?php if($row['is_open_mobilemanage']==1){ ?>checked<?php } ?> lay-skin="switch" lay-text="开启|关闭">
											<?php }else{?>
												<input type="checkbox" name="" lay-filter="wsitchmobilemanage" disabled="true" lay-skin="switch" lay-text="开启|关闭">
											<?php } ?>
        							...
        							<td style="overflow:visible;text-align: center;">
                                            ...
                                            <a class="layui-btn layui-btn-xs" href="<?php echo U('supply/addsupply',array('id'=>$row['id'], 'ok'=>1));;?>">
                                                <i class="layui-icon layui-icon-edit"></i>编辑
                                            </a>
											<?php if( $row['state'] == 0 && $row['goods_count'] <= 0 ){ ?>
        									
											 <a class='layui-btn layui-btn-xs deldom' href="javascript:;" data-href="<?php echo U('supply/deletesupply',array('id' => $row['id']) );?>" data-confirm='确认要删除吗?'>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="5">
                                        <input type='checkbox' name="checkall" lay-skin="primary" lay-filter="checkboxall" /> 
            							<div class="btn-group">
            								<a class='btn btn-default btn-sm btn-op btn-operation'  data-toggle='batch' data-href="<?php echo U('supply/agent_check',array('state'=>1));?>"  data-confirm='确认要审核通过?'>
            									<i class="icow icow-shenhetongguo"></i>审核通过
            								</a>
            								<a class='btn btn-default btn-sm btn-op btn-operation'  data-toggle='batch' data-href="<?php echo U('supply/agent_check',array('state'=>0));?>" data-confirm='确认要取消审核?'>
            									<i class="icow icow-yiquxiao"></i>取消审核</a>
            								<a style="display:none;" class="btn btn-default btn-sm btn-op btn-operation" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php echo U('communityhead/deletehead');?>">
            									<i class='icow icow-shanchu1'></i> 删除
            								</a>
            							</div>
                                    </td>
                                    <td colspan="<?php if($supply_is_open_mobilemanage == 1){ ?>7<?php }else{ ?>6<?php } ?>" style="text-align: right">
                                        <?php echo ($pager); ?>
                                    </td>
                                </tr>
                            </tfoot>
...
<script language="javascript">
   ...
   $("[data-toggle='ajaxModal']").click(function () {
		...
		ajax_url = s_url;
	   var s_url2 = $(this).attr('data-href2');
	   ajax_url2 = s_url2;
		console.log(23);
       $.ajax({
				url:s_url,
