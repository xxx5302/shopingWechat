...
<div class="layui-fluid">
								...
								<ul class="">
									...
									<li class="text">
										...
										<span class="text-default">
											...
											<?php if( $item['payment_code'] == 'supply_mobile'){ ?>供应商手机端付款<?php } ?>
										</span>
									</li>
									<li class="text">
										...
										<span class="text-default">
											<?php  if (defined('ROLE') && ROLE == 'agenter' ) { ?>
											 <?php echo ($member['username']); ?> &nbsp;&nbsp;会员ID ：<?php echo ($member['member_id']); ?>
											<?php }else{ ?>
												<?php if($s_id == 1){ ?>
													<a href="<?php echo U('user/detail',array('id'=>$member['member_id']));?>" target='_blank' class="text-primary"><?php echo ($member['username']); ?></a> 
												<?php }else{ ?>
													<a id="user" target='_blank' class="text-primary"><?php echo ($member['username']); ?></a> 
												<?php } ?>
											&nbsp;&nbsp;会员ID ：<?php echo ($member['member_id']); ?>
									...
									<?php if($is_order_note_open == 1){ ?>
									<?php if(!($item['delivery'] == 'express' && ($item['type'] == 'normal' || $item['type'] == 'pintuan'))){ ?>
									<?php if( !empty($item['note_content'])){ ?>
									<li class="text">
										<span style="padding-left: 10px;"><?php echo ($order_note_name); ?>：</span>
										<span class="text-default">
											 <?php echo ($item['note_content']); ?>
										</span>
									</li>
									<?php } ?>
									<?php } ?>
									...
									<li class="text">
										...
										<span class="text-default">
											...
											<?php }elseif( $item['delivery'] == 'tuanz_send' ){ ?>
												团长配送
											<?php  }elseif( $item['delivery'] == 'hexiao' ){ ?>
												到店核销
											<?php  }else{ ?>
												同城配送
											<?php } ?>
					...
					<div class=" order-container-static">
						<div class=" status">
							...
							<?php if( $item['order_status_id'] == 4){ ?>
								<span class="text-warning font18">已发货</span>
								<?php if($item['delivery'] == 'hexiao'){ ?>
									<span class="text-warning font18">待使用</span>
								<?php }else{?>
						...
						<?php if( $item['delivery'] == 'localtown_delivery'){ ?>
							<?php if( !empty($item['expected_delivery_time'])){ ?>
							<div>
								<ul>
									<!--
									<?php
 $weekarray=array("日","一","二","三","四","五","六"); $week = $weekarray[date('w',$item['date_added'])]; ?>
									<li class="text">期望配送时间：<span class="text-default"><?php echo date('m-d',$item['date_added'])?>（周<?php echo $week;?>）<?php echo ($item['expected_delivery_time']); ?></span></li>-->
									<li class="text">期望配送时间：<span class="text-default"><?php echo ($item['expected_delivery_time']); ?></span></li>
								</ul>
							</div>
							<?php } ?>
							<?php
 if( $item['order_status_id'] == 4 || $item['order_status_id'] == 6 || $item['order_status_id'] == 11 ){ if(!empty($item['orderdistribution_order']['username'])){ ?>
							<div>
								<ul>
									<li class="text">配送员：
										<span class="text-default"><a href="javascript:void(0);" style="color:#428bca;"><?php echo $item['orderdistribution_order']['username']; ?></a></span>
										<!--<?php if($item['orderdistribution_order']['orderdistribution_id'] == -1){?>
										<i class="layui-icon layui-icon-about font20" data-message="“批量发货--订单批量确认收货”系统默认配送员配送完成。" style="font-size: 20px;"></i>
										<?php }?>-->
									</li>
								</ul>
							</div>
						...
						<div>
							<td>
								<a href="<?php echo U('group/pintuan_detail', array('pin_id' => $pin_id));?>" target="_blank" class="btn btn-success btn-sm">查看拼团详情</a>
							</td>
						</div>
						<?php } ?>
						...
						<div class="ops  layui-col-md-12" style="padding: 0;">
							...
							<?php if($is_pay_show){ ?>
								<?php if(!isset($open_admin_payment) || $open_admin_payment == '1'){ ?>
							...
							<?php
 if (!defined('ROLE')){ if( $item['order_status_id'] == 3 && $item['type'] != 'integral'){ ?>
							<a class='op btn btn-primary btn-xs' data-toggle="ajaxModal" href="javascript:;" data-href="<?php echo U('order/change_order', array('id' => $item['order_id']));?>" >
								<i class="icow icow-yibiaoji" style="color: #999;display: inline-block;vertical-align: middle" title="订单改价" ></i>
								订单改价
							</a>
							<?php }} ?>
						...
						<div class="order-container-footer text col-md-12" style="border: none;padding: 0">
							...
							<?php if( $item['order_status_id'] == 4){ ?>
							<?php if($item['delivery'] == 'hexiao'){ ?>
							<span style="font-size:12px;color:#a7a5a5;">注：确认使用后，订单下面全部商品默认核销成功!</span>
							<br/>
							友情提示：如果无法进行发货，请及时联系买家进行妥善处理;
							<?php }else{?>
							友情提示：
				...
				<table class="layui-table ">
					...
					<tr class="trorder" style="background: #fff">
						...
						<th style="padding-left: 20px;width:10%;">规格、编号</th>
						<th style="text-align: left;padding-left: 20px;width:10%;">基数</th>
						<?php if( $item['type'] != 'integral' ){ ?>
						<th style="padding-left: 20px;width:15%;">团长佣金&nbsp;</th>
						<?php } ?>
						<th style="width:120px;">是否退款</th>
						<?php if($item['delivery'] == 'hexiao'){ ?>
						<th style="width:120px;">到店核销方式</th>
						<?php }?>
					...
					<?php $total_refund_shipping_fare = 0; ?>
					
					<?php $integral_total_refund_money = 0; ?>
					<?php $integral_total_refund_shipping_fare = 0; ?>
					
					
					<?php $order_total = 0; ?>
					<?php foreach( $order_goods as $goods ){ ?>
					    <?php
 if(!empty($goods['goods_refund_shipping_fare'])){ $total_refund_shipping_fare = $total_refund_shipping_fare + $goods['goods_refund_shipping_fare']; } ?>
						<?php $member_youhui += ($goods['oldprice'] - $goods['price'])*$goods['quantity']; ?>
				...
				overflow: hidden;text-overflow: ellipsis;"><?php echo ($goods['name']); if($goods['is_refund_state'] == 1 ){ ?>(退款)<?php } ?></a>
							...
							<td style="padding: 10px 20px">
								<?php  $vip_free_money = 0; $mblevel_free_money =0; if( $goods['is_level_buy'] == 1 ) { $mblevel_free_money = round(($goods['oldprice'] - $goods['price'])*$goods['quantity'],2); } if( $goods['is_vipcard_buy'] == 1 ) { $vip_free_money = round(($goods['oldprice'] - $goods['price'])*$goods['quantity'],2); } $order_total = $order_total + $goods['old_total']; $change_price = 0; if($goods['is_change_price'] == 1){ $change_price = $goods['price']*$goods['quantity'] - $goods['total']; } ?>
								 &nbsp;&nbsp;商品合计<?php echo ($goods['old_total']); ?><br/>
								 -&nbsp;满减:<?php echo ($goods['fullreduction_money']); ?><br/>
								 -&nbsp;优惠券：<?php echo ($goods['voucher_credit']); ?><br/>
								<?php if( ($goods['is_change_price'] == 1 && $change_price > 0 ) ){ ?>
								-&nbsp;商家改价：<?php echo ($change_price); ?><br/>
								<?php } ?>
								 <?php if( ($goods['is_level_buy']>0 && $mblevel_free_money > 0 ) ){ $item['is_level_buy'] = 1; ?>
								  -&nbsp;会员等级优惠：<?php echo ($mblevel_free_money); ?><br/>
								 <?php } ?>
								 <?php if( ($goods['is_vipcard_buy']>0 && $vip_free_money > 0 ) ){ $item['is_vipcard_buy'] = 1; ?>
								  -&nbsp;付费会员优惠：<?php echo ($vip_free_money); ?><br/>
								 <?php } ?>
								 <?php if($goods['has_refund_money']>0){ ?>
								 -&nbsp;已退款：<?php echo ($goods['has_refund_money']); ?><br/>
								 <?php } ?>
								 
								 =&nbsp;基数：<?php echo round($goods['total'] -$goods['fullreduction_money']-$goods['voucher_credit']-$goods['has_refund_money'],2); ?>
							...
							<td style="padding: 10px 20px">
									...
									<?php if($item['delivery']=='tuanz_send'){ ?>
										佣金：<?php if($vvv['money'] ==0 ){ echo round( $vvv['add_shipping_fare'],2); }else{ echo round($vvv['money'] - $vvv['add_shipping_fare'],2);} ?> </br>
										<?php if($vvv['level'] <= 0){ ?>
							...
							<td style="text-align: center;">
								...
								<?php }else if($goods['is_refund_state'] == 1 && !empty($goods['refund_info']) ){ ?>		
									退款金额：<?php echo $goods['refund_info']['ref_money']; ?>  <?php if($item['type'] == 'integral'){ ?>积分<?php }else{ ?>元<?php } ?>,
								...
								<?php  }else if($goods['is_refund_state'] == 0 && in_array( $item['order_status_id'], array(1,4,6,10,11,12,14)) && $is_can_nowrfund_order ){ ?>
									<?php if($item['delivery'] == 'hexiao'){ ?>
										<?php if(!empty($goods['hx_info']) && $goods['hx_info']['is_can_refund'] == 1){ ?>
									...
									<?php } ?>
									<?php if($goods['has_refund_quantity'] > 0){ ?>已退款商品数量<?php echo ($goods['has_refund_quantity']); ?>个,<br />退款<?php if($item['type'] == 'integral'){ ?>积分<?php }else{ ?>金额<?php } echo ($goods['has_refund_money']); if($item['type'] == 'integral'){ ?>积分<?php }else{ ?>元<?php } } ?>
								<?php }else{ ?>
									<?php if($goods['has_refund_quantity'] > 0){ ?>已退款商品数量<?php echo ($goods['has_refund_quantity']); ?>个,<br />退款<?php if($item['type'] == 'integral'){ ?>积分<?php }else{ ?>金额<?php } echo ($goods['has_refund_money']); if($item['type'] == 'integral'){ ?>积分<?php }else{ ?>元<?php } } ?>
								<?php } ?>
							</td>
							<?php if($item['delivery'] == 'hexiao'){ ?>
							<td style="text-align: center;">
								<?php if(!empty($goods['hx_info'])){ ?>
									<?php if($goods['hx_info']['hexiao_type'] == 0){ ?>
										<span>订单核销</span>
									<?php }else{ ?>
										<span>按次核销</span>
									<?php } ?>
									<br/>
									<?php if($item['order_status_id'] == 11 && $goods['hx_info']['is_hexiao_over'] == 1){ ?>
									<a class="hxBtn"  href="javascript:;" data-href="<?php echo U('order/view_hexiao_history', array('id' => $item['order_id'],'order_goods_id'=>$goods['order_goods_id']));?>">（核销记录）</a>
									<?php }?>

									<?php  if($item['order_status_id'] == 7 || $goods['is_refund_state'] == 1 || (!empty($goods['refund_info'] && $goods['refund_info']['state'] != 4))){ }else{ ?>
									<?php if($goods['hx_info']['is_hexiao_over'] == 1){ ?>
										<?php if($item['order_status_id'] == 4){ ?>
										<a class="hxBtn"  href="javascript:;" data-href="<?php echo U('order/view_hexiao_history', array('id' => $item['order_id'],'order_goods_id'=>$goods['order_goods_id']));?>">（已核销）</a>
										<?php } ?>
									<?php }else{ ?>
										<?php if($goods['hx_info']['hexiao_type'] == 0){ ?>
											<?php if($item['order_status_id'] == 4){ ?>
											<a class="btn btn-primary btn-xs deldom" href="javascript:;"   data-confirm="确认使用该订单吗？" data-href="<?php echo U('order/order_goods_hexiao', array('id' => $item['order_id'],'order_goods_id'=>$goods['order_goods_id']));?>">确认使用</a>
											<?php } ?>
										<?php }else{ ?>
											<?php if($item['order_status_id'] == 4){ ?>
												<a class="btn btn-primary btn-xs hxAction"  href="javascript:;" data-href="<?php echo U('order/hexiao_times', array('id' => $item['order_id'],'order_goods_id'=>$goods['order_goods_id']));?>">确认使用</a>
											<?php } ?>
											<?php if($goods['hx_info']['remain_hexiao_count'] > 0 && $goods['hx_info']['remain_hexiao_count'] < $goods['hx_info']['hexiao_count'] && $goods['hx_info']['hexiao_count'] > 0 ){ ?>
												<br/>
												<a class="hxBtn"  href="javascript:;" data-href="<?php echo U('order/view_hexiao_history', array('id' => $item['order_id'],'order_goods_id'=>$goods['order_goods_id']));?>">(部分核销)</a>
											<?php } ?>
											<?php if($goods['hx_info']['hexiao_count'] == 0 && $goods['hx_info']['record_count'] > 0){ ?>
												<br/>
												<a class="hxBtn"  href="javascript:;" data-href="<?php echo U('order/view_hexiao_history', array('id' => $item['order_id'],'order_goods_id'=>$goods['order_goods_id']));?>">（核销记录）</a>
											<?php } ?>
										<?php } ?>
							...
							<?php } ?>
							<td style="text-align: center;">
								<span><?php echo ($goods['supply_name']); ?><br/>(<?php echo ($goods['supply_type']); ?>)</span>
								<?php if($item['delivery'] == 'hexiao'){ ?>
								<?php  if(!empty($goods['hx_info']) && $goods['hx_info']['hx_assign_salesroom'] == 1){ ?>
									<br/>
									<a class="hxBtn"  href="javascript:;" data-href="<?php echo U('order/hexiao_goods_assign_salesroom', array('goods_id' => $goods['goods_id'],'order_goods_id'=>$goods['order_goods_id']));?>">(商品指定核销)</a>
								<?php } } ?>
							</td>
							...
							<td style="text-align: center">
								...
								<p><?php if($item['type'] == 'integral'){ ?> <?php }else{ ?>￥<?php } echo round($goods['oldprice'],2); if($item['type'] == 'integral'){ ?>积分<?php }else{ ?> <?php } ?>
								<!--
								<?php if( $goods['member_disc'] < 100 ){ ?>
								<span class="label label-primary">会员折扣：<?php echo ($goods['member_disc']); ?>%</span>
								<?php } ?>-->
								<?php if( $goods['is_vipcard_buy'] == 1){ ?>
									<span class="label label-primary">付费会员价格：<?php echo round($goods['price'],2);?></span>
								<?php }else if($goods['is_level_buy'] == 1){ ?>
									<?php if( $goods['member_disc'] < 100 ){ ?>
									<span class="label label-primary">会员折扣：<?php echo ($goods['member_disc']); ?>%</span>
									<?php } ?>
								<?php } ?>
							...
							<td  style="text-align: center"><?php if($item['type'] == 'integral'){ ?> <?php }else{ ?>￥<?php } echo round($goods['oldprice']*$goods['quantity'],2);?></td>
							
						</tr>
						<?php if($item['is_commission'] == 1 && !empty($goods['member_commission_list']) ){ ?>
						 <tr class="tr-order-sn">
							<td <?php if($item['delivery'] == 'hexiao'){ ?>colspan="11"<?php }else{ ?>colspan="10"<?php } ?>>
								分销佣金：<br/>
								...
								*&nbsp;数量：<?php echo ($goods['quantity']); ?>
								...
								分销员：<span style="display:inline-block;min-width: 70px;"><?php echo $mb_val['username']; ?></span>（<?php echo $mb_val['level']; ?>级上线）
								&nbsp;&nbsp;&nbsp;&nbsp;佣金比例：
								<?php if($mb_val['type'] == 1){ $jishu = $goods['total'] -$goods['fullreduction_money']-$goods['voucher_credit']-$goods['has_refund_money']; ?><span style="display:inline-block;width: 50px;"><?php echo $mb_val['bili'];?>%</span>  &nbsp;&nbsp;&nbsp;&nbsp;有效基数：<span style="display:inline-block;width: 50px;"><?php echo $jishu; ?></span>
								&nbsp;&nbsp;&nbsp;&nbsp;具体佣金：<?php echo round($jishu*$mb_val['bili']/100,2); ?>元 &nbsp;
								<?php }else{ ?>
								固定佣金<?php echo $mb_val['bili']; ?>
								&nbsp;*&nbsp;数量：<?php echo ($goods['quantity']); ?>
								=&nbsp;具体佣金：<?php echo round($mb_val['bili']*$goods['quantity'],2); ?>元 &nbsp;
								<?php } ?>
								<!--=&nbsp;具体佣金：<?php echo $mb_val['money']; ?>元 &nbsp;-->

								<?php  $str_state = ''; switch($mb_val['state']) { case 0: $str_state='待结算'; break; case 1: $str_state='已结算'; $order_status = 1; break; case 2: $str_state='订单退款'; break; } ?>(<?php echo $str_state; ?>)<br/>     
								<?php } ?>	
							</td>
						</tr>
						<?php } ?>
					...
					<tr class="trorder">
						<td colspan="2" style="padding-left: 20px"> 
							订单结算状态：
								<?php  $order_states = ''; switch($order_status) { case 0: $order_states='待结算'; break; case 1: $order_states='已结算'; break; case 2: $order_states='已失效'; break; } echo $order_states; ?>   
							<span onmouseover="overShow1()" onmouseout="outHide1()" style="display:inline;border-radius: 50%; height: 17px;width:17px; display: inline-block; border-style: solid; border-color: #8a8a8a; border-width:1px; font-size:11px"><span style="padding-left: 4px;font-weight:bold;color:#8a8a8a;">？</span></span>
						</td>
						<td  <?php if($item['type'] == 'integral'){ ?> colspan="7"<?php }else if($item['delivery'] == 'hexiao'){ ?> colspan="9"<?php }else{ ?>colspan="8"<?php } ?> style="padding-right: 60px"></td>
					</tr>
					<?php } ?>
					<tr class="trorder" style="height:100px;">
						<td colspan="2"> 
						<div id="showDiv" style="position:relative; background-color: #8a8a8a; display:none; height:80px;width:1200px;border-radius:20px;padding-left: 20px;color:#FFF;">
						-待结算：还在订单“售后期”。<br />
						-已结算：已过订单“售后期”。<br />
						-已失效：整单退款的订单。<br />
						<?php if(($item['order_status_id'] ==6 || $item['order_status_id'] ==11) ){ ?> 
							<?php if($order_status == 0 && $open_aftersale == 1){ ?> 
							-订单预计结算时间：<?php echo date("Y-m-d H:i:s",$order_settlement); ?>，订单结算状态为“已结算”的时候，用户在小程序订单中不能在申请商品售后，订单中的会员佣金，团长佣金为已结算状态。
							<?php }else{ ?> 
							-订单结算状态为“已结算”的时候，用户在小程序订单中不能在申请商品售后，订单中的会员佣金，团长佣金为已结算状态。
							<?php } ?> 
						<?php }else{ ?>
							...
							<?php }else{ ?> 
								...
								<?php if($open_aftersale == 1){ ?> 
								-订单预计结算时间：订单 “确认收货” 后，预计<?php echo $open_aftersale_time; ?>天后进行结算，订单结算状态为“已结算”的时候，用户在小程序订单中不能在申请商品售后，订单中的会员佣金，团长佣金为已结算状态。
								<?php }else{ ?> 
								-订单预计结算时间：订单 “确认收货” 后进行结算，订单结算状态为“已结算”的时候，用户在小程序订单中不能在申请商品售后，订单中的会员佣金，团长佣金为已结算状态。
								<?php } ?> 
							<?php } ?> 
						<?php } ?>
						</div>
						...
						<td  <?php if($item['type'] == 'integral'){ ?> colspan="7"<?php }else if($item['delivery'] == 'hexiao'){ ?> colspan="9"<?php }else{ ?>colspan="8"<?php } ?> style="padding-right: 60px">
							<div class="price">
								
								<p> <span class="price-inner">商品合计：</span><span style="font-weight: bold"><?php if( $item['type'] == 'integral' ){ }else{ ?>￥<?php } echo $order_total; if( $item['type'] == 'integral' ){ ?>积分<?php } ?></span></p>

								<?php if( $item['delivery'] == 'localtown_delivery'){ ?>
									<?php if( $item['is_localtown_free_shipping_fare'] == 1 ){ ?>
									<p><span class="price-inner">同城配送费：</span>￥<?php echo ($item['localtown_free_shipping_fare']); ?></p>
									<p><span class="price-inner">满减配送费：</span>-￥<?php echo ($item['localtown_free_shipping_fare']); ?></p>
									<?php }else{ ?>
									<p><span class="price-inner">同城配送费：</span>￥<?php echo ($item['shipping_fare']); ?></p>
									<?php  } ?>
									<?php if( !empty($item['localtown_add_shipping_fare']) && $item['localtown_add_shipping_fare'] > 0 ){ ?>
									<p><span class="price-inner">加价配送：</span>￥<?php echo ($item['localtown_add_shipping_fare']); ?></p>
									<?php  } ?>
								<?php }else{ ?>
									<?php if( $item['is_free_shipping_fare'] == 1 ){ ?>
									<p><span class="price-inner">运费：</span>￥<?php echo ($item['fare_shipping_free']); ?><span>( <?php echo ($item['delivery_text']); ?>  )</span></p>
									<p><span class="price-inner">满<?php echo ($item['man_e_money']); ?>减运费：</span>-￥<?php echo ($item['fare_shipping_free']); ?></p>
									<?php }else{ ?>
									<p><span class="price-inner">运费：</span>￥<?php echo ($item['shipping_fare']); ?><span>( <?php echo ($item['delivery_text']); ?>  )</span></p>
									<?php  } ?>
								<?php  } ?>
								
								<?php if( $item['packing_fare']>0 ){ ?>
								<p><span class="price-inner"><?php echo ($localtown_modifypickingname); ?>：</span>￥<?php echo ($item['packing_fare']); ?></p>
								<?php } ?>
								...
								<?php if($item['is_change_price'] == 1){ ?>
								<p>
									<span class="price-inner">商家改价：</span>
									<span style=''>
										<?php
 $change_price = $item['total'] - $item['old_price']; if($change_price < 0){ echo '-￥'.(0-$change_price); }else{ echo '￥'.$change_price; } ?>
									</span>
								</p>
								<?php } ?>
 ...
 if( $item['is_level_buy'] == 1 && $member_youhui >0 ) { ?>	
								...
								<span style="font-size: 14px;font-weight: bold;color: #e4393c"><?php if( $item['shipping_fare'] > 0 ){ ?>￥<?php echo round($item['shipping_fare'],2);?>+<?php } ?> <?php echo round($item['total'],2);?> 积分</span>
									...
									<p><span class="price-inner">退款：</span><span style="font-size: 14px;font-weight: bold;color: #e4393c"><?php echo ($total_refund_money); ?>积分</span></p>
									<?php } ?>
									<?php
 if( $total_refund_shipping_fare > 0 ){ ?>
									<p><span class="price-inner">退运费：</span><span style="font-size: 14px;font-weight: bold;color: #e4393c">￥<?php echo ($total_refund_shipping_fare); ?></span></p>
									<?php } ?>
								
								<?php }else{ ?>
									
									<p><span class="price-inner">实付款：</span><span style="font-size: 14px;font-weight: bold;color: #e4393c">￥<?php echo round($free_tongji,2)?></span></p>
									<?php  if( $total_refund_money > 0 ){ ?>
