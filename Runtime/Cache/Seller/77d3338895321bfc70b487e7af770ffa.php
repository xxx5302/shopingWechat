<?php if (!defined('THINK_PATH')) exit();?><div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
  <ul class="layui-tab-title">
    <li class="layui-this">商城</li>
    <li>商品</li>
    <li>文章</li>
	<li>主题活动</li>
	 <li>分类</li>
	...
	<div class="tab-content " id="selectUrl">
		<div class="tab-pane active" id="sut_shop">
			<div class="input-group">
				<input type="text" placeholder="请输入链接名称进行搜索" id="select-url-kw" value="" class="form-control">
			...
			<?php foreach( $allUrls as $item ){ ?>
				<div class="layui-form-item " style="margin-top:10px;"><h4><i class="fa fa-folder-open-o"></i> <?php echo ($item['name']); ?></h4></div>
				<?php foreach( $item['list'] as $child ){ ?>
					<nav data-href="<?php echo $platform?$child['url_wxapp']:$child['url'];?>" class="choose_dan_link_url btn btn-default btn-sm" title=""><?php echo ($child['name']); ?></nav>
				<?php } ?>
			<?php } ?>
			</div>
		</div>
			...
			<?php foreach( $categorys as $category ){ ?>
				 <?php if( empty($category['parentid'])){ ?>
					<div class="line">
						...
						<nav title="选择" class="choose_dan_link_url btn btn-default btn-sm" data-href="/pages/goods/index/index?cate=<?php echo ($category['id']); ?>">选择</nav>
						<div class="text"><?php echo ($category['name']); ?></div>
					</div>
					 <?php foreach( $categorys as $category2 ){ ?>
						<?php if( $category2['parentid']==$category['id']){ ?>
							<div class="line">
								<div class="icon icon-2"></div>
								<nav title="选择" class="btn btn-default btn-sm" data-href="/pages/goods/index/index?cate=<?php echo ($category2['id']); ?>">选择</nav>
								<div class="text"><?php echo ($category2['name']); ?></div>
							</div>
							<?php foreach( $categorys as $category3 ){ ?>
								 <?php if( $category3['parentid']==$category2['id']){ ?>
									 <div class="line">
										<div class="icon icon-3"></div>
										<nav title="选择" class="btn btn-default btn-sm" data-href="/pages/goods/index/index?cate=<?php echo ($category3['id']); ?>">选择</nav>
		...
		<div class="tab-pane" id="sut_special">
			<div class="input-group">
				<input type="text" placeholder="请输入活动名称进行搜索" id="select-special-kw" value="" class="form-control">
				<span class="input-group-addon btn btn-default select-btn" data-type="special">搜索</span>
			</div>
		...
		<div class="tab-pane" id="sut_category">
			<div class="input-group">
				<input type="text" placeholder="请输入分类名称进行搜索" id="select-category-kw" value="" class="form-control">
				<span class="input-group-addon btn btn-default select-btn" data-type="category">搜索</span>
			</div>
			<div id="select-category-list"></div>
		</div>
		<div class="tab-pane" id="sut_solitaire">
			<div class="input-group">
				<input type="text" placeholder="请输入群接龙名称进行搜索" id="select-solitaire-kw" value="" class="form-control">
				<span class="input-group-addon btn btn-default select-btn" data-type="solitaire">搜索</span>
			</div>
		...
		<div class="tab-pane" id="sut_pintuan">
			<div class="input-group">
				<input type="text" placeholder="请输入拼团商品名称进行搜索" id="select-pintuan-kw" value="" class="form-control">
				<span class="input-group-addon btn btn-default select-btn" data-type="pintuan">搜索</span>
			</div>
...
<style>
		...
		#selectUrl .layui-form-item {padding: 9px 0; margin-bottom: 8px;}
		#selectUrl .layui-form-item h4 {margin: 0;}
 ...
 <script>
        	$(function(){
        		...
        		$(".select-btn").click(function(){
        			...
        			$.ajax("<?php echo U('util/query', array('full'=>$full, 'platform'=>$platform, 'ok' => 1));?>", {
		      			type: "get",
		      			dataType: "html",
		      			cache: false,
